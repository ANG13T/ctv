...
fn main() -> anyhow::Result<()> {
    ...
    if !protocols::checkconfig::check_config(&config_input)? {
        anyhow::bail!("Config variables not declared properly");
    }
    ...
    let mut dir_tree = protocols::DirTree::init(args.dir, &check_config);
    dir_tree.gen()
}

fn modify_config_with_flags(
    config_input: &protocols::configmanager::ConfigManager,
) -> anyhow::Result<protocols::configmanager::ConfigManager> {
...
fn check_valid_set_var(
    ...
    let (env_key, env_value) = match env_input.split_once('=') {
        Some(split) => split,
        None => anyhow::bail!("ERROR: invalid flag variable for --set-var"),
    };
